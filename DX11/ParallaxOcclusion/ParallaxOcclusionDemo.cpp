//***************************************************************************************
// ParallaxOcclusionDemo.cpp by Frank Luna (C) 2012 All Rights Reserved.
//
// Demonstrates normal mapping, parallax occlusion, and displacement mapping.
//
// Controls:
...
enum RenderOptions
{
	...
	RenderOptionsParallaxOcclusion = 2,
	RenderOptionsDisplacementMap = 3
};

class ParallaxOcclusionApp : public D3DApp 
{
public:
	ParallaxOcclusionApp(HINSTANCE hInstance);
	~ParallaxOcclusionApp();
...
private:
	...
	float mStoneMaxHeight;
	float mBrickMaxHeight;
	float mHeightScale;
				   ...
				   PSTR cmdLine, int showCmd)
{
	...
	ParallaxOcclusionApp theApp(hInstance);
...
ParallaxOcclusionApp::ParallaxOcclusionApp(HINSTANCE hInstance)
: D3DApp(hInstance), mSky(0), 
  ...
  mSkullIndexCount(0),  mRenderOptions(RenderOptionsNormalMap), mModeText(L"NormalMap Rendering")
{
	mMainWndCaption = L"Normal-Parallax-Displacement Map Demo";
	
	mLastMousePos.x = 0;
	mLastMousePos.y = 0;

	mStoneMaxHeight = 0.3f;
	mBrickMaxHeight = 0.1f;
	mHeightScale    = 0.5f;
	...
	XMStoreFloat3(&mDirLights[0].Direction, XMVector3Normalize(XMVectorSet(1, -1.0f, 1, 0.0f)));
...
ParallaxOcclusionApp::~ParallaxOcclusionApp()
{
	SafeDelete(mSky);
...
bool ParallaxOcclusionApp::Init()
{
	if(!D3DApp::Init())
		return false;
	...
	HR(mFont.Initialize(md3dDevice, L"Times New Roman", 24.0f, FontSheet::FontStyleRegular, true));
	...
	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, 
		L"Textures/stones.dds", 0, 0, &mStoneTexSRV, 0 ));

	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, 
		L"Textures/bricks.bmp", 0, 0, &mBrickTexSRV, 0 ));
...
void ParallaxOcclusionApp::OnResize()
{
	D3DApp::OnResize();
...
void ParallaxOcclusionApp::UpdateScene(float dt)
{
	//
	// Control the camera.
	//
	...
	if( GetAsyncKeyState('2') & 0x8000 )
	{
		...
		mModeText = L"Basic Rendering";
	}

	if( GetAsyncKeyState('3') & 0x8000 )
	{
		mRenderOptions = RenderOptionsNormalMap; 
		mModeText = L"NormalMap Rendering";
	}

	if( GetAsyncKeyState('4') & 0x8000 )
	{
		mRenderOptions = RenderOptionsParallaxOcclusion; 
		mModeText = L"ParallaxOcclusion Rendering";
	}

	if( GetAsyncKeyState('5') & 0x8000 )
	{
		mRenderOptions = RenderOptionsDisplacementMap; 
		mModeText = L"DisplacementMap Rendering";
	}

	//
	// Adjust displacement map heights.
	//
	if( GetAsyncKeyState('F') & 0x8000 )
	{
		mHeightScale += 0.5f*dt;
	}
	...
	mHeightScale = MathHelper::Clamp(mHeightScale, 0.0f, 1.0f);
}

void ParallaxOcclusionApp::DrawScene()
{
	...
	Effects::ParallaxFX->SetDirLights(mDirLights);
	Effects::ParallaxFX->SetEyePosW(mCam.GetPosition());
	Effects::ParallaxFX->SetCubeMap(mSky->CubeMapSRV());
	...
	case RenderOptionsNormalMap:
		...
		md3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		break;
	case RenderOptionsParallaxOcclusion:
		activeTech = Effects::ParallaxFX->Light3TexTech;
		md3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		break;
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		case RenderOptionsBasic:
			...
			Effects::BasicFX->SetTexTransform(XMMatrixScaling(10.0f, 10.0f, 1.0f));
		...
		case RenderOptionsNormalMap:
			...
			Effects::NormalMapFX->SetTexTransform(XMMatrixScaling(10.0f, 10.0f, 1.0f));
			...
			break;
		case RenderOptionsParallaxOcclusion:
			Effects::ParallaxFX->SetWorld(world);
			Effects::ParallaxFX->SetWorldInvTranspose(worldInvTranspose);
			Effects::ParallaxFX->SetWorldViewProj(worldViewProj);
			Effects::ParallaxFX->SetTexTransform(XMMatrixScaling(10.0f, 10.0f, 1.0f));
			Effects::ParallaxFX->SetMaterial(mGridMat);
			Effects::ParallaxFX->SetDiffuseMap(mStoneTexSRV);
			Effects::ParallaxFX->SetNormalMap(mStoneNormalTexSRV);
			Effects::ParallaxFX->SetMinSampleCount(8);
			Effects::ParallaxFX->SetMinSampleCount(72);
			Effects::ParallaxFX->SetHeightScale(mStoneMaxHeight*mHeightScale*10.0f/30.0f);
		...
		case RenderOptionsDisplacementMap:
			...
			Effects::DisplacementMapFX->SetTexTransform(XMMatrixScaling(10.0f, 10.0f, 1.0f));
			...
			Effects::DisplacementMapFX->SetHeightScale(mStoneMaxHeight*mHeightScale);
			Effects::DisplacementMapFX->SetMaxTessDistance(5.0f);
			Effects::DisplacementMapFX->SetMinTessDistance(50.0f);
		...
		Effects::ParallaxFX->SetMinSampleCount(8);
		Effects::ParallaxFX->SetMinSampleCount(72);
		Effects::ParallaxFX->SetHeightScale(mBrickMaxHeight*mHeightScale/3.0f);

		Effects::DisplacementMapFX->SetHeightScale(mBrickMaxHeight*mHeightScale);
		Effects::DisplacementMapFX->SetMaxTessDistance(5.0f);
		Effects::DisplacementMapFX->SetMinTessDistance(50.0f);
		Effects::DisplacementMapFX->SetMinTessFactor(1.0f);
		Effects::DisplacementMapFX->SetMaxTessFactor(8.0f);
		...
		case RenderOptionsParallaxOcclusion:
			...
			Effects::ParallaxFX->SetWorldInvTranspose(worldInvTranspose);
			Effects::ParallaxFX->SetWorldViewProj(worldViewProj);
			Effects::ParallaxFX->SetTexTransform(XMMatrixScaling(1.0f, 1.0f, 1.0f));
			Effects::ParallaxFX->SetMaterial(mBoxMat);
			Effects::ParallaxFX->SetDiffuseMap(mBrickTexSRV);
			Effects::ParallaxFX->SetNormalMap(mBrickNormalTexSRV);
			break;
		case RenderOptionsDisplacementMap:
			Effects::DisplacementMapFX->SetWorld(world);
			Effects::DisplacementMapFX->SetWorldInvTranspose(worldInvTranspose);
		...
		for(int i = 0; i < 10; ++i)
		{
			...
			case RenderOptionsParallaxOcclusion:
				...
				Effects::ParallaxFX->SetWorldInvTranspose(worldInvTranspose);
				Effects::ParallaxFX->SetWorldViewProj(worldViewProj);
				Effects::ParallaxFX->SetTexTransform(XMMatrixScaling(1.0f, 2.0f, 1.0f));
				Effects::ParallaxFX->SetMaterial(mCylinderMat);
				Effects::ParallaxFX->SetDiffuseMap(mBrickTexSRV);
				Effects::ParallaxFX->SetNormalMap(mBrickNormalTexSRV);
				break;
			case RenderOptionsDisplacementMap:
	...
	POINT textPos = {10, 10};
	mSpriteBatch.DrawString(md3dImmediateContext, mFont, mModeText, textPos, XMCOLOR(0xffffffff));
...
void ParallaxOcclusionApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void ParallaxOcclusionApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void ParallaxOcclusionApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void ParallaxOcclusionApp::BuildShapeGeometryBuffers()
{
	...
	geoGen.CreateBox(1.0f, 1.0f, 1.0f, 3, box);
	geoGen.CreateGrid(30.0f, 30.0f, 100, 80, grid);
	geoGen.CreateSphere(0.5f, 20, 20, sphere);
	geoGen.CreateCylinder(0.5f, 0.5f, 3.0f, 10, 10, cylinder);
...
void ParallaxOcclusionApp::BuildSkullGeometryBuffers()
{
